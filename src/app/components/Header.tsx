...
export default function Header() {
    ...
    return (
        <header className="fixed top-0 left-0 w-screen px-8 pt-4 pb-8 flex flex-col lg:items-center justify-between lg:flex-row bg-verde">
            ...
            <nav className={`${!openMenu ? 'translate-x-full lg:translate-x-0' : 'translate-x-0'} transform transition-transform duration-300 ease-out z-40 bg-preto p-8 flex flex-col rounded-bl-lg text-verdeBX fixed w-80 h-screen right-0 top-0 space-y-4 border-l border-b lg:w-max lg:flex-row lg:justify-end lg:space-y-0 lg:grow lg:relative lg:bg-transparent lg:h-min lg:p-0 lg:border-none`}>
                <button type="button"
                    className="mt-4 text-4xl text-branco self-end lg:hidden"
                ...
                {navLinks.map(link => (
                    <a key={link.path} 
                        href={link.path} 
                        className={`border-b border-branco text-2xl px-4 hover:font-bold ${pathname == link.path ? 'text-branco' : ''} lg:text-sm lg:border-none ${!openMenu ? 'hidden lg:inline-block' : ''}`}
                    >
                        {link.title}
                    </a>
                ))}
